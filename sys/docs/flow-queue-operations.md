...
Với ảnh cảnh kế thừa (`based_on`) mà media ID gốc thuộc profile trước, queue-runner chỉ thử chuyển **ảnh nền đã lưu đủ byte và có nhật ký `generated`/`collected` khớp hash**. Trước khi gửi bất kỳ ảnh đang chờ nào ở profile mới, nó tải ảnh nền vào project của profile đó, yêu cầu cùng một media ID xuất hiện trong phản hồi upload thật và tile UI, lưu screenshot/hash cùng cặp ID nguồn→đích ở `results/controller/reference-transfers/`. Upload đã bắt đầu nhưng thiếu ID/chứng cứ trở thành `REFERENCE_TRANSFER_RECONCILIATION_REQUIRED`; tuyệt đối không tự upload hoặc gửi ảnh lại. Ảnh nhân vật chưa có ánh xạ vẫn dừng vì upload một ảnh thường chưa chứng minh đã đăng ký Character. Cấu hình `tool_url`/đăng nhập của profile mới và việc lấy media ID từ Flow UI vẫn cần nghiệm thu chạy thật; kiểm thử giả lập không chứng minh chức năng này hoạt động trên tài khoản.

`machine.local.json` ghi đè `browser-profiles.json`. Các profile kế tiếp phải có `tool_url`, ảnh nhân vật đã ánh xạ với bằng chứng và đúng đường dẫn profile Chrome. Cấu hình bật chuyển tài khoản không tự chứng minh upload ảnh nền hay media ID đã hoạt động thật; cần nghiệm thu UI riêng, không suy từ unit test. Thiếu URL, đăng nhập, CAPTCHA, ID upload không đối chiếu được hoặc ảnh nguồn thiếu nhật ký byte thì hàng đợi dừng an toàn.
